"""
Django settings for NETMIN project.
...
ROOT_URLCONF = "netmin.urls"
WSGI_APPLICATION = "netmin.wsgi.application"
...
PROJECT_METADATA = {
    ...
    'KEYWORDS': 'pdns, powerdns, netmin, admin, manage, console, dns, domain, nameserver, recursor, cache, authoritative, '
                + 'dnssec, app, ui',
...
TEMPLATES = [
    {
        ...
        'DIRS': [str(Path(settings.template_path) / 'jinja'), ],
        'APP_DIRS': False,
        'OPTIONS': {
            'environment': 'netmin.jinja2.JinjaEnvironment',
            'context_processors': [
    ...
    },
    {
        ...
        "DIRS": [str(Path(settings.template_path) / 'django'), ],
        "APP_DIRS": True,
        "OPTIONS": {
            "context_processors": [
                "django.template.context_processors.debug",
